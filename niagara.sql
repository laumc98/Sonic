...
FROM
    puzzles.opportunity AS o
    LEFT JOIN (
        SELECT
            opportunity_reference_id,
            LISTAGG(channel,', ') WITHIN GROUP (ORDER BY opportunity_reference_id) as channels,
            MIN(created) as channels_open_date
        FROM
            puzzles.opportunity_channels
        WHERE
            active = 1
        GROUP BY
            opportunity_reference_id
    ...
    LEFT JOIN (
        ...
        FROM
            puzzles.views AS v
            LEFT JOIN puzzles.applications AS a ON v.gg_id = a.gg_id
    ...
    LEFT JOIN (
        ...
        FROM
            puzzles.views AS v
            LEFT JOIN puzzles.opportunity AS opp ON v.opportunity_reference_id = opp.ref_id
            LEFT JOIN puzzles.applications AS a ON v.gg_id = a.gg_id
            AND v.opportunity_reference_id = a.opportunity_reference_id
...
WHERE
    o.created >= current_date - interval '365 days'
    AND lower(o.opportunity_snapshot) LIKE '%"crawled": false%'