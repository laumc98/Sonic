...
SELECT
   ...
   -- Created date 
    ...
    -- DR 
    (select p.name from opportunity_members omp left join people p on omp.person_id = p.id where omp.tso_operator = TRUE and omp.opportunity_id = o.id limit 1) as 'Ticket Owner',
    -- Commited date
